...
func TestLoadDependencies(t *testing.T) {
	...
	assert.Equal(t, "dep_id_1", dependencies.DependenciesByService["kafka"][0].GetID(), "Dependency ID should be correct")
	assert.Equal(t, 0, len(dependencies.DependenciesByService["kafka"][0].Labels), "First dependency should have 0 labels")
	assert.Equal(t, 2, len(dependencies.DependenciesByService["kafka"][1].Labels), "Second dependency should have 1 label")
