...
func TestPolicyVisualizationEmptyToNonEmpty(t *testing.T) {
	if testing.Short() {
		t.SkipNow()
	}
...
func TestPolicyVisualizationNonEmptyToEmpty(t *testing.T) {
	if testing.Short() {
		t.SkipNow()
	}
	...
	// create visualization engine from diff
	vis := NewPolicyVisualization(diff.NewRevisionDiff(revisionEmpty, revisionNew))

	// generate images
	{
		imagePrev, err := vis.GetImageForRevisionPrev()
		assert.Nil(t, err, "Image should be generated")
		assert.True(t, imagePrev.Bounds().Dx() > 800, "Image for unit test resolved policy should be big enough")
		assert.True(t, imagePrev.Bounds().Dy() > 800, "Image for unit test resolved policy should be big enough")
...

	{
		imageNext, err := vis.GetImageForRevisionNext()
		assert.Nil(t, err, "Image should be generated")
		assert.True(t, imageNext.Bounds().Dx() < 20, "Image for empty revision should be empty")
		assert.True(t, imageNext.Bounds().Dy() < 20, "Image for empty revision should be empty")
