...
aptomi_binary_dir="$(cd $(dirname "$0") && pwd)"
...
function log() {
    set +x
    echo -e "$COLOR_BLUE[$(date +"%F %T")] internal-watch-apply $COLOR_RED|$COLOR_RESET" $@$COLOR_GRAY
    if [ "yes" == "$DEBUG" ] ; then
        set -x
    fi
}
...
pushd $tmp 1>/dev/null
    ...
    while sleep $interval ; do
        ...
        if [ "$current_head" != "$remote_head" ]; then
            ...
            if $aptomi_binary_dir/aptomi policy apply --noop ; then
                log "Running aptomi policy apply"
                if ! $aptomi_binary_dir/aptomi policy apply --newrevision; then
